function [results_corr_avg_cond_body,results_corr_avg_cond_body_table] = IBS_vicon_manipulated_video_corr(conditions,varargin_table)
%IBS_VICON_VIDEO_MANIPULATED_CORR computes the correlation between the vicon and (manipulated) video data
%
% SYNOPSIS: derived from IBS_vicon_video_corr
%
% INPUT
%
% OUTPUT
...
if nargin <1
    ...
    varargin_table.manipulation = 'distance';
end
if nargin <2
    varargin_table = table();
    varargin_table.manipulation = 'distance';
...
try varargin_table.Dyads;
    Dyads = varargin_table.Dyads;
catch
    Dyads = 1:23;
end
...
savefname_mat = ['E:\\Projects\\IBS\\Results\\Vicon\\vicon_video_corresp\\vicon_video_corresp_manipulated_' varargin_table.manipulation '_' cond_name '_all_sub.mat'];
...
if exist(savefname_mat,'file')
    ...
    results_corr_avg_cond_body_table = array2table(results_corr_avg_cond_body','VariableNames',body_parts);

else
    results_corr = cellfun(@(body_part) arrayfun(@(Dyads) arrayfun(@(sub) cellfun(@(cond) IBS_compute_corresp_vicon_manipulated_video(Dyads,sub,cond,body_part,varargin_table),...
        conditions,'UniformOutput',0),0:1,'UniformOutput',0),Dyads,'UniformOutput',0),body_parts,'UniformOutput',0);
...
savefname = ['E:\\Projects\\IBS\\Results\\Vicon\\vicon_video_corresp\\figures\\vicon_' varargin_table.manipulation '_video_corresp_all_sub.tif'];

if exist(savefname,'file')
else
    % plot
    n_sub = size(results_corr_avg_cond_body,2);%46;
    n_bodyparts = size(results_corr_avg_cond_body,1);%12;
